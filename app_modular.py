...
# Legacy JSON Server Service Class (deprecated)
class LegacyJSONServerService:
    """
...
from modules.handlers import (
    ...
    download_ar_apk, step1_handler, step2_handler, step3_handler, step4_handler,
    test_debug_handler
)
...
# Global JSON server service instance (using new integrated service)
json_server_service = JSONServerService.get_instance()
...
def create_app():
    ...
    # Register template context functions
    @app.context_processor
    def utility_processor():
        from modules.handlers import get_transport_name
        return dict(get_transport_name=get_transport_name)
    ...
    # Step-based routes
    app.route('/step1', methods=['GET', 'POST'])(step1_handler)
    ...
    def handle_global_error(error):
        ...
        if 'filename' in str(error):
            print("🔍 FILENAME ERROR CAUGHT BY GLOBAL HANDLER!")
        ...
        # For regular requests, render error page or return JSON
        return jsonify({'error': f'An unexpected error occurred: {str(error)}'}), 500

    # Debug test route
    app.route('/debug_test')(test_debug_handler)
    ...
    def start_json_server_handler():
        """Start the JSON server for AR visualization by running json_server.py"""
        try:
            app.logger.info("AR View: JSON server start requested")
            
            # Path to the json_server.py script
            script_path = os.path.join(os.path.dirname(__file__), 'json_server.py')
            
            if not os.path.exists(script_path):
                app.logger.error(f"json_server.py script not found at {script_path}")
                return jsonify({
                    'success': False,
                    'error': 'json_server.py script not found'
                }), 400
            
            # Check if JSON server is already running on port 8000
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    result = s.connect_ex(('localhost', 8000))
                    if result == 0:
                        app.logger.info("JSON server is already running on port 8000")
                        return jsonify({
                            'success': True,
                            'url': f"https://{AppConfig.NGROK_DOMAIN}/{AppConfig.STANDARD_JSON_FILENAME}",
                            'message': 'JSON server is already running'
                        })
            except Exception as e:
                app.logger.warning(f"Error checking port 8000: {e}")
                pass
            
            # Start the json_server.py script as a subprocess
            app.logger.info("Starting json_server.py script...")
            ...
            if process.poll() is not None:
                ...
                app.logger.error(f"JSON server process failed: {stderr_output}")
                return jsonify({
                    'success': False,
                    'error': f'JSON server failed to start: {stderr_output}'
                }), 400
            
            # Store process reference globally for cleanup
            if not hasattr(app, '_json_server_process'):
                app._json_server_process = []
            app._json_server_process.append(process)
            
            app.logger.info(f"JSON server started successfully (PID: {process.pid})")
            return jsonify({
                'success': True,
                'url': f"https://{AppConfig.NGROK_DOMAIN}/{AppConfig.STANDARD_JSON_FILENAME}",
                'message': 'JSON server started successfully'
    ...
    def stop_json_server_handler():
        ...
        try:
            app.logger.info("AR View: JSON server stop requested")
            
            # Kill any running json_server.py processes
            stopped_any = False
            
            # Check if we have stored process references
            if hasattr(app, '_json_server_process'):
                for process in app._json_server_process:
                    if process.poll() is None:  # Process is still running
                        app.logger.info(f"Terminating JSON server process (PID: {process.pid})")
                        process.terminate()
                        try:
                            process.wait(timeout=5)
                            app.logger.info(f"JSON server process {process.pid} terminated gracefully")
                        except subprocess.TimeoutExpired:
                            app.logger.warning(f"JSON server process {process.pid} did not terminate, killing...")
                            process.kill()
                        stopped_any = True
                app._json_server_process.clear()
            
            # Also try to find and kill any remaining processes
            try:
                if os.name == 'nt':  # Windows
                    # Find python processes running json_server.py
                    result = subprocess.run(
                        ['tasklist', '/FI', 'IMAGENAME eq python.exe', '/FO', 'CSV'],
                        capture_output=True, text=True
                    )
                    # Kill ngrok processes as well
                    ngrok_result = subprocess.run(['taskkill', '/F', '/IM', 'ngrok.exe'], 
                                 capture_output=True)
                    if ngrok_result.returncode == 0:
                        app.logger.info("Killed ngrok processes")
                    stopped_any = True
                else:  # Unix/Linux/Mac
                    subprocess.run(['pkill', '-f', 'json_server.py'], 
                                 capture_output=True)
                    subprocess.run(['pkill', 'ngrok'], 
                                 capture_output=True)
                    stopped_any = True
            except Exception as e:
                app.logger.warning(f"Could not kill processes: {e}")
            
            message = 'JSON server stopped successfully' if stopped_any else 'No JSON server was running'
    ...
    def check_json_server_status_handler():
        ...
        try:
            # Check if the JSON server is running on port 8000
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    result = s.connect_ex(('localhost', 8000))
                    if result == 0:
    ...
    # Add cleanup handler for JSON server processes
    @app.teardown_appcontext
    def cleanup_json_server(error):
        """Clean up JSON server processes when app shuts down"""
        if hasattr(app, '_json_server_process'):
            for process in app._json_server_process:
